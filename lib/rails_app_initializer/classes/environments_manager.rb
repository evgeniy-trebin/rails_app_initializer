  ...
  class EnvironmentsManager
    ...
    def update_environments
      %w(development production test).each do |env|
        File.open(path_to_environment(env), 'w') do |f|
          f.write send("code_for_#{env}")
        end
      end
    end

    def path_to_environment(env)
      default_path("config/environments/#{env}.rb")
    end

    private

    def code_for_development
      <<-CONTENT
